...
## 4. Verification
- Rebuild the paper and check:
  - the main body falls in the **20-25 page** band
  - cross-references still resolve after renumbering
  - each expanded section contains at least one concrete artifact: a figure, a worked example, or a proof sketch
- Check that the Bayes-ball example and the reverse proof sketch only reference lemmas already present in the codebase.
- Check that no section is being padded artificially; if a section naturally stays short, let it stay short.

## Assumptions
- SIGPLAN/PL norms justify a fuller exposition than ACM `sigconf` security-style papers.
- References and appendix remain outside the main-page budget.
- The point is not to inflate length, but to lower exposition density enough that the full proof story is readable to a POPL reviewer.
</proposed_plan>
