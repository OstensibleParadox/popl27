...
node sets. In the current artifact, this affine ownership constraint is enforced
\emph{intrinsically} via dependent type signatures—ensuring that downstream
semantic pipelines are fundamentally type-safe by construction. To present the
same core \emph{extrinsically}, we plan a first-order surface query language
(no higher-order binders) with an evaluator and a separate typechecker; surface
graphs carry a rank certificate for acyclicity (checked in linear time) and are
