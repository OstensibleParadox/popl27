...
\begin{abstract}
We give a type-safe route from qualitative d-separation to certified
Shannon-style leakage bounds.  The route has four links:
d-separation, conditional independence, conditional data processing, and a
cut-set bound.  This arXiv artifact mechanizes the graph-semantic front end in
Lean~4; a separate anonymized artifact mechanizes the finite quantitative
information-flow chain and is cited anonymously \cite{companionqif}.
...
contains two intentional scaffold stubs that restate results proved in the
anonymized companion artifact.
\end{abstract}
\maketitle

\section{Introduction}\label{sec:intro}

The target is a certified route from a graph query to a quantitative
...
\]
This arXiv artifact contains the graph-semantic part of that route.  A
separate anonymized artifact \cite{companionqif} proves the semantic
d-separation-to-CI bridge, conditional Markov instances, cut-set bounds,
capacity certificates, and linear-chain DAG instances.  The local bridge file
in this repository is therefore a scaffold restating companion-proved results,
not an in-package proof of the quantitative chain.
...
\begin{enumerate}
  ...
  \item We connect the graph core to an anonymized quantitative
    information-flow artifact \cite{companionqif}, which proves the semantic
    bridge \lean{factorizes_dsep_implies_cond_indep}, cut-set theorem
    \lean{abstract_cut_set_bound}, KKT capacity certificate
    \lean{capacity_le_of_kkt}, and end-to-end DAG-routed instance
    \lean{linear_chain_cut_set_bound_from_dag}.  This arXiv artifact does not
    import that project; its local \lean{InfoTheoryBridge.lean} file is an
    explicit scaffold restating the companion-proved bridge.
\end{enumerate}
...
The companion artifact proves \lean{factorizes_dsep_implies_cond_indep} under
a stated \lean{FactorizesOverDAG} hypothesis; the structural Verma--Pearl
derivation of that hypothesis from a DAG factorization theorem remains a
technical debt, not a gap in the semantic bridge theorem.  It also proves
...
hypothesis, and the Gaussian-KL steps are carried as explicit external
premises (\lean{AssumesFanoBound}, \lean{AssumesMissedCellBound}).  The
remaining open items are listed in Appendix~\ref{app:depplan}.
...
debt.  The local probabilistic bridge scaffold restates companion-proved
results; what remains is the narrow technical debt listed below.
...
\begin{enumerate}
  \item \textbf{Structural Verma--Pearl derivation.}  The companion artifact
  ...
  \item \textbf{General capacity metatheorem.}  The companion artifact proves
