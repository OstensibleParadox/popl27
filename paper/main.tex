...
For the extrinsic presentation, we target a first-order surface calculus with
an explicit environment and no higher-order binders, so the metatheory stays
focused on queries rather than substitution-heavy closure machinery.  Surface
graphs carry a rank certificate for acyclicity, checked locally and then
...
\lean{dSeparation_}\allowbreak\lean{implies_}\allowbreak\lean{conditional_}\allowbreak\lean{independence} in
\lean{InfoTheoryBridge.lean}, together with stubs for \lean{FinitePMF} and
\lean{MarkovCompatible}.  This scaffold is deliberately \emph{not} discharged
in this artifact: it carries two intentional \lean{sorry}s and is the sole
proof debt in the package.  The finite-measure and information-theory
