...
concrete counterexample.  It shows that the equivalence fails when an endpoint
node lies in the conditioning set~$Z$; we identify the repair
($\text{DisjointSets}\,X\,Y\,Z$) and prove the corrected equivalence.
...
To bridge from structural d-separation to quantitative leakage bounds, our
...
Both developments adopt standard mathematical formulations without exposing the implicit textbook domain caveats. Our work contributes a formalized counterexample and repair for the gap between syntactic trails and moralized node-deletion. We also move from purely relational verification to a constructive equivalence that generates executable IRs (e.g., \lean{OpenTrace}) in Lean~4 for witness synthesis.
