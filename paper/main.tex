...
\lstset{
  ...
  literate={_}{{\_\allowbreak}}1 {.}{{\allowbreak.}}1,
  columns=flexible,
...
analysis of moralized-graph reachability.  Our formalization is organized in three layers: a dependently-typed graph semantics enforcing affine ownership constraints ($\text{DisjointSets}\,X\,Y\,Z$) \emph{intrinsically}, a
...
independence is treated not as a missing proof but as a \emph{formal interface contract} (\lean{InfoTheoryBridge.lean}), following the pattern of hardware abstraction layers in verified operating systems.
\end{abstract}
...
Consider the simplest possible directed acyclic graph: a chain $0 \to 1 \to 2$.  Let $X = \{0\}$, $Y = \{1\}$, and $Z = \{0\}$.  Does $Z$ d-separate $X$ from $Y$?
The two standard textbook characterizations---trail blocking~\cite{pearl2009causality} and moralized ancestral-graph separation~\cite{lauritzen1996graphical}---give different answers for this case. This discrepancy arises from the treatment of endpoints in $Z$, a detail often omitted in equivalence proofs.

We mechanize this discrepancy and its repair, treating the two
characterizations as a \emph{verified bisimulation} between an
operational semantics of active trails (execution) and a static analysis of
moralized-graph reachability (compilation). The forward direction is a
\emph{certified trace optimizer} that compiles traces into static reachability; the
reverse direction is a \emph{constructive decompiler} that reconstructs explicit
trail witnesses from static evidence.

\subsection{Contributions}

Our contributions are:
\begin{enumerate}
  \item We formalize both d-separation criteria in Lean 4 as a three-layer stack (Section~\ref{sec:architecture}): **intrinsic graph semantics** enforcing affine ownership, a **type-indexed IR** (\lean{MAGWalk}, \lean{StaticRoute}), and **constructive witness pipelines**.
  \item We machine-check a concrete counterexample identifying the endpoint treatment as the sole source of Restricted Equivalence failure (Section~\ref{sec:counterexample}).
  \item We mechanize the **forward bisimulation** (soundness): a compilation pipeline $\text{Trail} \to \text{MAGWalk} \to \text{Reachable}$, certified by node-survival proofs (Section~\ref{sec:forward}).
  \item We mechanize the **reverse bisimulation** (witness-producing completeness): a decompiler reconstructing unblocked trails from reachability evidence via a proved **bad-collider normalization** (\lean{route_improves_of_bad}) (Section~\ref{sec:reverse}).
  \item We close the **full equivalence** (\lean{dSeparated_iff_dSeparates}) on the pairwise-disjoint domain; the graph-semantic core carries no proof debt.
  \item We define a **formal interface contract** (\lean{InfoTheoryBridge.lean}) to delimit the boundary to quantitative information flow (Section~\ref{sec:related}).
\end{enumerate}
...
of a verified compiler. To orient the reader, we summarize the mapping between causal-graph vocabulary and programming-languages terms in Table~\ref{tab:dictionary}.
...
\subsection{Layer 1: Intrinsic Graph Semantics}
...
node sets. In the current artifact, this affine ownership constraint is enforced \emph{intrinsically} via dependent type signatures—ensuring that downstream semantic pipelines are fundamentally type-safe by construction. Developing an explicit surface syntax calculus (with mechanized Progress and Preservation for dynamic queries) over this verified substrate is a focus of our ongoing work.
...
The proof follows the three-stage compilation pipeline shown in Figure~\ref{fig:pipeline}.
...
The pipeline follows the stages summarized in Figure~\ref{fig:pipeline}.
...
leaked ancestry must reach a node in $X$ or $Y$. The core intuition is that since the bad child $w$ is part of the ancestral subgraph of $X \cup Y \cup Z$, but its descendant cone misses $Z$, it cannot ``dead-end''; it must eventually escape to the source $X$ or sink $Y$. \lean{bad_child_survives} and
...
\lean{countBadColliders_backwardEscape_append_suffix_lt} (resp.\
\lean{countBadColliders_prefix_append_forwardEscape_lt}) and \lean{omega}.  As a
...
Formalizing the graph-theoretic foundations of causal inference is an active frontier. Recently, \emph{Chancelier, De Lara, and Heymann (2024)} \cite{chancelier2024conditional} mechanized d-separation as a binary relation using the Coq proof assistant and MathComp/SSReflect, providing a robust relational-algebraic treatment of graphical separation. Parallel to our work, \emph{London Bielicke (2025)} \cite{bielicke2025formalizing} formalized the semantics of conditional independence and d-separation in Coq, elegantly bridging structural properties with semantic function-based definitions for experimental design. 

While these Coq developments provide phenomenal milestones for the semantics of causal graphical models, they predominantly adopt the traditional mathematical formulations without questioning the implicit textbook domain caveats. To our knowledge, our work provides the first formalized \emph{counterexample} and repair (the endpoint caveat) addressing the exact gap between syntactic trails and moralized node-deletion. We also shift the methodology from purely relational verification to a \emph{constructive bisimulation} generating executable IRs (e.g., \lean{OpenTrace}), specifically optimized in Lean 4 for computational witness synthesis.

\paragraph{Categorical and Topological D-Separation.}
From a purely mathematical perspective, \emph{Fritz and Klingler (2023)} \cite{fritz2023dseparation}, extending foundational work by \emph{Fritz and Liang}, abstracted d-separation using string diagrams in Markov categories. Their elegant framework bypasses moralization altogether, reading conditional independence directly from topological connectedness in a generalized, measure-theoretic space. Our approach is complementary: while Markov categories provide a transcendent topological theory, our artifact targets the \emph{legacy operational graph definitions} (moral graphs and trace trails) universally implemented in existing software systems. By defining \lean{DisjointSets} as an affine ownership type, we reveal that classical d-separation behaves exactly like a resource-restricted syntax within a broader categorical structure.

\paragraph{Verified Trace Bisimulation and Witness Extraction.}
Framing graphical reasoning as a compiler verification problem draws deep inspiration from standard PL literature. The forward pipeline (soundness) mimics the semantics-preserving translations pioneered by \emph{Leroy's CompCert (2009)} \cite{leroy2009formal}, translating unbounded trace semantics into a compressed step-relation (\lean{MAGWalk}). Conversely, our reverse pipeline (completeness)—which deterministically unwinds reachability into a certified active trail—is philosophically aligned with \emph{Necula’s Proof-Carrying Code (PCC) (1997)} \cite{necula1997proof}. Instead of simply returning a boolean assertion of an information leak, our decompiler synthesizes an explicit, machine-checkable exploitability certificate. 

This establishes a foundational graph-theoretic layer for future Verified Static Analyzers evaluating Quantitative Information Flow (QIF). We treat the bridge to probabilistic conditional independence not as a missing proof but as a \emph{formal interface contract} (\lean{InfoTheoryBridge.lean}), following the pattern of hardware abstraction layers in verified operating systems. This contract specifies the exact graph-semantic properties required to license probabilistic independence:
\[
\text{d-separation} \;\Rightarrow\; \text{conditional independence}
\;\Rightarrow\; \text{conditional DPI} \;\Rightarrow\; \text{cut-set bound}.
\]
We state the first step of this bridge as an explicit Lean scaffold,
\lean{dSeparation_implies_conditional_independence}, together with stubs for \lean{FinitePMF} and
\lean{MarkovCompatible}.  This scaffold is deliberately \emph{not} discharged
in this artifact: it serves as the boundary between graph-semantic verification and measure-theoretic probability. The finite-measure and information-theory
...
DAG semantics formalized here is the principal item of future work.
