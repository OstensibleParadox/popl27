...
constructive except for one normalization lemma whose proof debt is isolated
...
\begin{enumerate}
  ...
  \item We mechanize the \textbf{reverse bisimulation} (completeness): a
    ...
    Every stage is type-valued and constructive; the only remaining proof debt
    is isolated in a single normalization lemma.
\end{enumerate}
...
The remaining proof debt is isolated in the improvement lemma:

\begin{lstlisting}[caption={The sole remaining proof debt}]
theorem route_improves_of_bad
    (w : StaticRouteWitness G X Y Z) (hbad : routeBadCount w \neq 0) :
    \exists w' : StaticRouteWitness G X Y Z,
      routeBadCount w' < routeBadCount w := by
  sorry   -- isolated proof debt
\end{lstlisting}
