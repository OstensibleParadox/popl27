...
Chancelier, De Lara, and Heymann~(2024) \cite{chancelier2024conditional} mechanized d-separation as a binary relation using Coq and MathComp/SSReflect. Anna Zhang~(2025) \cite{coqci2025} proves the semantic equivalence in Coq with \lean{member u Z = false} and \lean{member v Z = false} already built into the theorem statement, while \lean{CausalQIF}~\cite{causalqif2026} proves the set-wise graph equivalence \lean{dSeparated_iff_dSeparates} under the explicit \lean{disjointSets X Y Z} hypothesis. Our work differs in scope: it proves semantic propagation of active trails, then normalizes certificates and feeds them into the QIF bridge.
...
conditional on factorization, defining the clean interface boundary between
